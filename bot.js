...
if (msg.content === 'selamın aleyküm') {
  ...
  if (msg.content === 'görüşürüz') {
   ...
   const tmm = client.emojis.find(emoji => emoji.name === "nitro");
   msg.reply(`${ayy} Bir nitro yükle, ${tmm} ile kalkışa geç.`);
  }
  
  if (msg.content === "!tag") {
    const tmm = client.emojis.find(emoji => emoji.name === "tmm")
    .setColor(0x00C09A)
    .addField('Tagımızı almak mı istedin', msg.author, ' Buyrun ifinimm')
    .addField(tmm, 'Tagımız: Ж')
  }
});
...
app.listen(8000);
setInterval(() => {
  http.get(`http://wexlica.glitch.me/`);//Buraya glitch linkinizi doğru şekilde giriniz. ve Botunuz 7/24 olacaktır!
}, 280000)
